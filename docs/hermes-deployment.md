...
Гайд написан по результатам анализа боевого хоста Hermes (Ubuntu 24.04) и
репозитория `hermes` от 2026-07-16. Все пути и ограничения ниже — проверенные
факты, не предположения. Адрес хоста и реквизиты доступа намеренно не
публикуются: репозиторий публичный.
