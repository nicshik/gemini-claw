...
The login itself is standard Google OAuth via Google's official `agy` client — gemini-claw's own code never sees your Google password and never handles the long-lived OAuth token. That is the honest headline, and it is also where the reassurance ends: once a host is logged in, account security reduces to host security, and the caveats below are real. Read them before you point a bot at your Google account.

**Who logs in.** Only the operator authenticates, once per host, through a browser (`scripts/login.sh`). End users of the Telegram bot never sign in with their own Google account — every `/antigravity`, `ask`, and `image` runs the operator's already-authenticated `agy` under the service user. The flip side is that there is no per-user identity or isolation: every authorized sender shares the operator's single AI Pro account and draws down the same quota, and the only trust boundary for them is OpenClaw's sender authorization (`dmPolicy` / pairing), not any Google login.

**What the login touches.** `login.sh` reads the auth URL that the official `agy` prints; you open it in your local browser, sign in on `accounts.google.com`, and paste the resulting authorization code back. Your password is entered only on Google's page — it never enters gemini-claw. The script does relay the one-time authorization code you paste (a short-lived grant, not the token) into the local `agy`, but it is never stored, logged, or sent off-box, and `agy` alone exchanges it for the actual tokens and writes them under `~/.gemini`. The repo never reads, copies, or transmits those tokens. Telemetry is turned off by default (best-effort), and a post-login check warns you if `agy` flipped it back on — but that check only warns; it does not re-disable telemetry, so you must turn it off again yourself. One gotcha: the code prompt is a plain read — the pasted value is echoed and only length-checked — so do not paste your password there by mistake.

**No hidden exfiltration.** The plugin and skill scripts carry no HTTP client and no covert telemetry: the plugin only spawns the local `agy` binary, and the skills only `subprocess` it. The one place your prompt or image description goes is Google — `agy` sends it to Gemini/Antigravity over Google's own OAuth channel, which is the product's whole function, not a leak — and replies and generated images go back only to the originating Telegram chat via OpenClaw's own outbound adapter. A CI `secret-scan.sh` gate backs the "no credentials in the repo" rule, but treat it as a backstop, not a guarantee: it reliably catches API keys and GitHub/Telegram/private keys, yet it does not recognize a Google OAuth token in its normal JSON form, skips gitignored and binary files entirely, and runs in CI after a push rather than as a local pre-commit block. What actually keeps credentials out of the repo is that the token only ever lives host-local in the service user's home, never in the tracked tree — though "host-local" only means the repo never syncs it between hosts; it is a portable bearer token, not cryptographically bound to the machine (see below).

**Residual risks and mitigations.**

- **Use a dedicated Google account, not your main one.** Everything runs under a single Google identity, at whatever OAuth scope `agy` requests (you do not get to narrow it), and driving one personal AI Pro account programmatically on behalf of others may bump into Google's terms of service and risks rate-limits or suspension. Log in with a dedicated AI Pro account so a revocation or ban does not touch your primary identity.
- **Treat the host as single-tenant and trusted.** The token is a portable bearer credential with no at-rest encryption added by this repo. Anyone with root — or a shell as the service user — can read it under `~/.gemini` and replay it against Google from any machine until you revoke it. Account security therefore equals host security; if a host is compromised, revoke at Google rather than just deleting the file.
- **Restrict who can message the bot.** Because `/antigravity_ask` hands sender text verbatim to an agent that runs as the service user with file tools — read as well as write — in that user's home, anyone allowed to DM the bot drives that agent on your server. That means more than spending your quota: a crafted prompt from any allowed sender could read the OAuth credential under `~/.gemini` and exfiltrate it. Keep the allowed-sender list tight (`dmPolicy` / pairing) — an open bot effectively lends strangers your Google identity.
- **Install is `curl | sudo bash` — read it first.** Setup fetches this repo from GitHub and runs as root, then fetches Google's official `agy` installer and runs it (and every later `agy` call) as the unprivileged service user, never as root. Both are unpinned `curl | bash` with no checksum or signature, so trust rests on TLS — read `scripts/bootstrap.sh` and `scripts/install.sh` before piping them into a root shell. The only step that genuinely cannot be automated is the Google browser sign-in.
